...
main <- function() {
  ...
  #wordcloud(em, min.freq = 50, rot.per=0.35,  colors=brewer.pal(8, "Dark2"))
  #  wordcloud(freq.to.h$word, freq.to.h$freq, min.freq = 20, random.order = FALSE, colors=brewer.pal(8, "Dark2"))
  #  wordcloud(freq.from.h$word, freq.from.h$freq, min.freq = 20, random.order = FALSE, colors=brewer.pal(8, "Dark2"))
...
wordFrequencyFor <- function(text) {
  ...
  my.stop <- c(stopwords("english"), "also", "will")
